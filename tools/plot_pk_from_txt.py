...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Plot two P(k) tables with ratio panel")
    parser.add_argument("--pk-a", required=True, help="Path to numerator spectrum txt")
    parser.add_argument("--pk-b", required=True, help="Path to denominator spectrum txt")
...
def main() -> None:
    ...
    k_ratio, ratio_ab = prepare_ratio(k_a, pk_a, k_b, pk_b)
    ...
    ax2.plot(k_ratio, ratio_ab, label=f"{args.label_a} / {args.label_b}", marker="o", markersize=3, linewidth=1.2)
    ax2.axhline(1.0, color="k", lw=0.8, linestyle="--")
    ax2.set_xlabel("k [h/Mpc]")
    ax2.set_ylabel("P(k)_A / P(k)_B")
    ax2.set_xscale("log")
