...
DELIBERATE local duplicate of unet/dataset.py (same file, byte-for-byte at the time
of writing) -- see feedback-decoupled-pipeline-modules memory: ml/ pipeline
directories don't import across each other so a rename in one can't silently break
another. This pipeline reads the SAME on-disk patch format unet's make_patch_dataset.py
produces (see this dir's own make_patch_dataset.py, also a deliberate duplicate), so
--patch-dir can point at an already-built unet patch directory without rebuilding it.

Returns *raw* counts, not the log1p(overdensity) representation the model actually
trains on - that transform is applied batched, on GPU, in the training loop (see
raw_to_log1p_delta_pair below) instead of per-sample in __getitem__.

Train/val split is by cosmology (not by patch index) so validation cosmologies are
never seen in training - a random per-patch split would leak, since many patches
from the same cosmology/run look highly correlated at large scales.
"""
...
# the exact field order DenoiserUNet's cosmology+redshift conditioning expects (see
# model.py). H0 -> h (only h=H0/100 is stored; same convention as the rest of this
# project). Omega_cdm isn't separately stored either -- only Om (total matter) and
# Ob (baryons) are -- so it is derived as Om - Ob.
...
def cosmo_z_vector(cosmo, z):
    ...
    w0, z]. The ONE place this ordering is defined -- train_diffusion.py and
    apply_diffusion.py both call this instead of re-deriving the field order
    themselves, so training and evaluation can never silently drift apart."""
...
def raw_to_log1p_delta_pair(low_raw: torch.Tensor, high_raw: torch.Tensor):
    ...
    (low_log, high_log), the log1p(overdensity) representation the model trains on."""
    low_mean = low_raw.mean(dim=(2, 3), keepdim=True)
    high_mean = high_raw.mean(dim=(2, 3), keepdim=True)
    ...
    def __getitem__(self, i):
        ...
        return {
            ...
            "cosmo": torch.from_numpy(cosmo),               # (Om,Ob,ns,s8,w0,h) -- see
                                                             ...
                                                             # DenoiserUNet actually consumes it
