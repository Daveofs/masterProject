...
def main():
    ...
    # --- SAFETY CHECK: Check for poisoned weights early ---
    has_nan_weights = any(torch.isnan(p).any() for p in model.parameters())
    if has_nan_weights:
        raise ValueError(
            "\n[!] FATAL: The loaded model contains NaN weights. "
            "It diverged during training. You must delete the model and retrain."
        )
    ...
    for batch_start in range(0, len(indices), args.batch_size):
        ...
        if args.diagnostic and batch_start == 0:
            print(f"\n  [DIAGNOSTICS] Initial State -> Max Val: {torch.abs(x_curr).max().item():.3e}")

        # Removed torch.amp.autocast! We must stay in FP32 to handle unnormalized a_lm variance.
        with torch.no_grad():
            for step in range(args.steps):
                t_tensor = torch.full((B,), step * dt, dtype=torch.float32, device=device)
                
                # Check for NaNs before pushing through the model
                if torch.isnan(x_curr).any():
                    print(f"    [!] FATAL: x_curr contains NaNs BEFORE inference at step {step}.")
                    break
                ...
                if args.diagnostic and batch_start == 0:
                    ...
                    x_max = torch.abs(x_curr).max().item()
                    has_nan = torch.isnan(x_curr).any().item() or torch.isnan(v).any().item()
                    
                    print(f"    Step {step:02d} | x_norm: {x_norm:.2e}, x_max: {x_max:.2e} | "
                          f"v_norm: {v_norm:.2e}, v_max: {v_max:.2e}")
                    
                    if has_nan:
                        print("    [!] FATAL: NaN generated by model prediction. Breaking loop.")
                        break
