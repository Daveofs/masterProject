...
def main():
    ...
    ap.add_argument("--demo-shells", type=int, nargs="+", default=[5, 20, 35, 50])
    ...
    for n, r in enumerate(runs):
        ...
        for s in range(len(chi)):
            ...
            if np.isfinite(lon):
                chi_all.append(chi[s]); l_all.append(lon); z_all.append(z[s]); keep += 1
    ...
    axL.axhline(1.0 - a.eps, color=C_TH, ls="--", lw=1.2)
    axL.text(0.985, 1.0 - a.eps, f"  onset level $1-\\epsilon$ ($\\epsilon={a.eps:.0%}$)",
             transform=axL.get_yaxis_transform(), ha="right", va="bottom",
             fontsize=FS_LEGEND - 1, color=C_TH)
    ...
    axR.plot(chi_all, l_all, "o", color=C_PTS, ms=4.0, alpha=0.35, mec="none",
             label=f"{len(l_all)} (cosmology, shell) pairs")
    xg = np.linspace(0, chi_all.max() * 1.05, 50)
    ...
    axR.set_xlim(0, chi_all.max() * 1.05); axR.set_ylim(0, a.l_hi * 1.05)
