...
\end{equation}
with $n_s$ the scalar spectral index \cite{huterer_growth_2023, eisenstein_baryonic_1998}. The transfer function $T(k)$ can in turn be approximated by
\begin{equation}
    T(k) = 1 - \frac{k}{k_{\mathrm{eq}}} + \dots, \qquad k_{\mathrm{eq}} = H_0\sqrt{\frac{2\Omega_m}{\Omega_r}},
\end{equation}
\cite{bardeen_statistics_1986, eisenstein_baryonic_1998}. The transfer function is therefore an essential ingredient of the matter power spectrum, alongside the matter and radiation densities. It also depends on the neutrino mass, as illustrated in Figure~\ref{fig:transfer_function}.
\begin{figure}[h!]
    \centering
...
\end{figure}
The three red curves correspond to the neutrino masses $m_{\nu_1}=0$, $m_{\nu_2}=0.009$ and $m_{\nu_3}=0.05$~eV \cite{lesgourgues_neutrino_2012, lesgourgues_massive_2006}. This in turn affects the angular power spectrum $C_\ell$, which is defined as
\begin{equation}
    C_\ell \approx \int k^2\, P(k)\, \mathrm{d}k,
    \label{eq:cl}
\end{equation}
and is essentially the Fourier transform of the matter power spectrum \cite{seljak_line_1996}.
\begin{figure}[h!]
    \centering
...
\end{figure}
This is fundamental for the present project, since its influence reappears later when generating the initial conditions for the simulations.
\vspace{\sectionvspace} \\
...
As described in Section~\ref{sec:power_spectrum}, the initial conditions must be built from a correct transfer function evaluated at the correct neutrino mass, since this directly determines whether the resulting field is consistent with CosmoGridV1.
\vspace{\sectionvspace} \\
...
Every Python script is paired with a Slurm batch script. The Python entry point is written to run unchanged both locally and on the cluster, while the accompanying batch script requests the resources and captures the output. The same job can then be launched interactively (\texttt{bash}, for fast monitoring directly in the terminal) or submitted to the queue (\texttt{sbatch}, to run detached and log to file).
...
\end{itemize}
A representative submission script is shown below.
...
Because only the header changes between runs, adapting a job to a new configuration -- more nodes, a longer time limit, or a different output directory -- amounts to editing a few directives, while the Python driver is reused unchanged.
